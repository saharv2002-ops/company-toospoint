"""Build the iCall × Anthropic annotation-vendor pitch as a 16:9 PowerPoint
backed by the rendered slide images, with hyperlink hot-spots that mirror the
HTML deck.
"""
import os
...
OUT     = os.path.join(SRC_DIR, 'iCall_Anthropic_Deployment_Partner_Pitch.pptx')
...
HOTSPOTS = {
    1: [
        # "See The Layer We Own" CTA pill (bottom-right)
        (0.665, 0.832, 0.260, 0.080, 'mailto:info@icallinternational.com'),
    ],
    5: [
        ...
        # "Book A Scoping Call" CTA button (centre)
        (0.42, 0.78, 0.30, 0.110, 'mailto:info@icallinternational.com'),
        # logo (right)
...
def main():
    ...
    cp.title    = 'iCall International — Multilingual Deployment Partner Pitch for Anthropic'
    cp.author   = 'iCall International'
    cp.subject  = 'Multilingual AI assurance, deployment and human-escalation partner for Claude at enterprise scale'
    cp.keywords = 'Claude, Anthropic, multilingual, assurance, deployment, human escalation, interpretation, OPI, VRI, regulated, healthcare, partner network'
